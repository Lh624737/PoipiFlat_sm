...
public class MaxNO {
    ...
    public static String getMaxNo(Context context) {
        ...
        if (orderDtos.size() == 0) {
            maxNo = getPhoneMac()+GetData.getYYMMDDNoSpellingTime()+ getDriverId() + "0001";

        } else {
            String m = String.valueOf(Long.parseLong(orderDtos.get(0).getMaxNo()) + 1);
            //流水号
            String number = m.substring(m.length() - 4, m.length());
            maxNo = GetData.getYYMMDDNoSpellingTime() +getDriverId()+ number;
        }
