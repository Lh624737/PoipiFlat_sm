...
//					delete(item);
    ...
    @OnClick({R.id.menu, R.id.vip_icon, R.id.scan_iv, R.id.scan_qc, R.id.scan_sure, R.id.more_payway, R.id.pay, R.id.ll_table})
...
//            vp.setAdapter(adapter);
            ...
            //判断是否为称重码
//            if (code.length() == 13) {3
              /*  if (code.substring(0, 2).equals("22")) {
                    //此条形码为称重码，获取商品码和价格码
                    double price = Double.parseDouble(code.substring(8, 11) + "." + code.substring(11, 13));
                    String goods_code = code.substring(2, 8);
                    goodsDto = goodsDao.selectGoods(goods_code);
                    Log.i("judgeCode", "goods_code: " + goods_code);
                    Log.i("judgeCode", "price: " + price);
                    if (goodsDto != null) {
